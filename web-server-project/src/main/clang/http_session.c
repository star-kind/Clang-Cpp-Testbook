...
int http_session(int *connect_fd, struct sockaddr_in *client_addr) {
    ...
    /* store the the uri request from client */
...
int is_http_protocol(char *msg_from_client) {
    ...
    if (strncmp(msg_from_client + index - 10, "HTTP/", 5) ==
        0)    /* HTTP Request firt line like this 'GET /index.html HTTP/1.1' , so last 10 byte are HTTP/1.1\r\n*/
    {
        return 1;
    }
...
char *get_mime_type(char *uri) {
    ...
    if (dot ==
        0)        /* if the uri begain with a dot and the dot is the last one, then it is a bad uri request,so return NULL */
    {
        return NULL;
    }
